...
void debugInit(uint32_t baudrate)
{
   ...
   SCIF3.SCFCR.WORD |= SCIF3_SCFCR_TFRST | SCIF3_SCFCR_RFRST;

   //Read serial status register
   status = SCIF3.SCFSR.WORD;
   //Clear ER, BRK and DR flags
   SCIF3.SCFSR.WORD = status & ~(SCIF3_SCFSR_ER | SCIF3_SCFSR_BRK | SCIF3_SCFSR_DR);

   //Read line status register
   status = SCIF3.SCLSR.WORD;
   //Clear ORER flag
   SCIF3.SCLSR.WORD = status & ~SCIF3_SCLSR_ORER;

   //Select internal clock
   SCIF3.SCSCR.WORD &= ~SCIF3_SCSCR_CKE;

   //Configure UART (8 bits, no parity, 1 stop bit)
   SCIF3.SCSMR.WORD = 0;

   //Enable double-speed mode
   SCIF3.SCEMR.WORD = SCIF3_SCEMR_BGDM;

   //Set baudrate
   SCIF3.SCBRR.BYTE = PCLK1_HZ / (16 * baudrate) - 1;
   ...
   SCIF3.SCSCR.WORD = SCIF3_SCSCR_TE | SCIF3_SCSCR_RE;
}
...
int_t fputc(int_t c, FILE *stream)
{
   ...
   if(stream == stdout || stream == stderr)
   {
      ...
      while(!(SCIF3.SCFSR.WORD & SCIF3_SCFSR_TDFE));
      //Send character
      SCIF3.SCFTDR.BYTE = c;
      //Clear TDFE flag
      SCIF3.SCFSR.WORD &= ~SCIF3_SCFSR_TDFE;
