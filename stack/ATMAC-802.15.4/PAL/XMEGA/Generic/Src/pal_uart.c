 ...
 *
 * $Id: pal_uart.c 31108 2012-03-02 07:45:33Z viswanadham.kotla $
 *
...
#if (defined UART2)
uart_communication_buffer_t uart_2_buffer;
#endif
...
void sio_uart_1_init(uint32_t baud_rate)
{
     ...
     * run at the given baud rate.
     */
	usart_set_baudrate(&UART1_REG, baud_rate, F_CPU);

	/* Faster async mode (UART clock divider = 8, instead of 16) */
    /* Enable Rx and Tx */
    //UART1_REG.CTRLB = USART_RXEN_bm | USART_TXEN_bm | USART_CLK2X_bm;
    UART1_REG.CTRLB = USART_RXEN_bm | USART_TXEN_bm;
...
#ifdef UART2
/**
 * @brief Initializes UART 2
 *
 * This function initializes the UART channel 2.
 *
 * @param baud_rate Actual UART baud rate
 */
void sio_uart_2_init(uint32_t baud_rate)
{
    /* Init Port F pins */
    /* PF3 (TXD1) as output. */
    UART2_PORT.DIRSET = UART2_TX_PIN;
    /* PF2 (RXD1) as input. */
    UART2_PORT.DIRCLR = UART2_RX_PIN;

    /* pull up configuration for UART Rx pin  */
    UART2_PORT.PIN2CTRL |= 0x18;
    /*
     * Microcontroller's USART register is updated to
     * run at the given baud rate.
     */
	usart_set_baudrate(&UART2_REG, baud_rate, F_CPU);

	/* Faster async mode (UART clock divider = 8, instead of 16) */
    /* Enable Rx and Tx */
    //UART2_REG.CTRLB = USART_RXEN_bm | USART_TXEN_bm | USART_CLK2X_bm;
    UART2_REG.CTRLB = USART_RXEN_bm | USART_TXEN_bm;


    /* Set 8N1  */
    UART2_REG.CTRLC = USART_CHSIZE1_bm | USART_CHSIZE0_bm;

    /*
     * Receive and transmit interrrupt are enabled.
     */
    UART2_REG.CTRLA = USART_RXCINTLVL_gm | USART_TXCINTLVL_gm;
}
#endif  /* UART2 */
...
#ifdef UART2
/**
 * @brief Transmit data via UART 2
 *
 * This function transmits data via UART channel 2.
 *
 * @param data Pointer to the buffer where the data to be transmitted is present
 * @param length Number of bytes to be transmitted
 *
...
uint8_t sio_uart_2_tx(uint8_t *data, uint8_t length)
{
    ...
    /* The transmit interrupt is disabled. */
    DISABLE_UART_2_TX_INT();

    /*
     * Calculate available buffer space
     */
    head = uart_2_buffer.tx_buf_head;
    tail = uart_2_buffer.tx_buf_tail;
    ...
    while (bytes_to_be_written > 0)
    {
        uart_2_buffer.tx_buf[uart_2_buffer.tx_buf_tail] = *data;

        if ((UART_MAX_TX_BUF_LENGTH - 1) == uart_2_buffer.tx_buf_tail)
        {
            /* Reached the end of buffer, revert back to beginning of buffer. */
     ...
     * bytes / data will be taken care in the ISR.
     */
    if (uart_2_buffer.tx_count == 0)
    {
        UART2_REG.DATA = uart_2_buffer.tx_buf[head];
        uart_2_buffer.tx_count = 1;
    }

    /* The transmit interrupt is enabled. */
    ENABLE_UART_2_TX_INT();
...
#ifdef UART2
/**
 * @brief Receives data from UART 2
 *
 * This function receives data from UART channel 2.
 *
 * @param data pointer to the buffer where the received data is to be stored
 * @param max_length maximum length of data to be received
 *
...
uint8_t sio_uart_2_rx(uint8_t *data, uint8_t max_length)
{
    ...
    /* The receive interrupt is disabled. */
    DISABLE_UART_2_RX_INT();

    if (UART_MAX_RX_BUF_LENGTH <= uart_2_buffer.rx_count)
    {
        /*
         * Bytes between head and tail are overwritten by new data.
         * The oldest data in buffer is the one to which the tail is
         * pointing. So reading operation should start from the tail.
         */
        uart_2_buffer.rx_buf_head = uart_2_buffer.rx_buf_tail;

        /*
         * This is a buffer overflow case. But still only bytes equivalent to
         * full buffer size are useful.
         */
        uart_2_buffer.rx_count = UART_MAX_RX_BUF_LENGTH;
    ...
    {
        /* Bytes received is less than receive buffer maximum length. */
        if (max_length > uart_2_buffer.rx_count)
        {
            /*
             * Requested receive length (max_length) is more than the data
             * present in receive buffer. Hence only the number of bytes
             * present in receive buffer are read.
             */
            max_length = uart_2_buffer.rx_count;
        }
    }
    ...
    {
        /* Start to copy from head. */
        *data = uart_2_buffer.rx_buf[uart_2_buffer.rx_buf_head];
        uart_2_buffer.rx_buf_head++;
        uart_2_buffer.rx_count--;
        data++;
        max_length--;
        if ((UART_MAX_RX_BUF_LENGTH) == uart_2_buffer.rx_buf_head)
        {
...
#ifdef UART2
/**
 * @brief ISR for UART 2 receive interrupt
 *
 * This service routine is executed when a byte is received successfully on
 * UART channel .
 */
ISR(UART2_RX_ISR_VECT)
{
    uint8_t tail = uart_2_buffer.rx_buf_tail;

    /* Count of bytes received through UART 2 channel is incremented. */
    uart_2_buffer.rx_count++;

    uart_2_buffer.rx_buf[tail] = UART2_REG.DATA;

    if ((UART_MAX_RX_BUF_LENGTH - 1) == uart_2_buffer.rx_buf_tail)
    {
         /* Revert back to beginning of buffer after reaching end of buffer. */
...
/**
 * @brief ISR for UART 2 transmit interrupt
 *
 * This service routine is executed when a byte is transmitted successfully on
 * UART channel 2.
 */
ISR(UART2_TX_ISR_VECT)
{
    if ((UART_MAX_TX_BUF_LENGTH - 1) == uart_2_buffer.tx_buf_head)
    {
        /* Reached the end of buffer, revert back to beginning of buffer. */
    ...
    else
    {
         ...
         * transmitted.
         */
        uart_2_buffer.tx_buf_head++;
    }

    if (uart_2_buffer.tx_buf_head != uart_2_buffer.tx_buf_tail)
    {
        UART2_REG.DATA = uart_2_buffer.tx_buf[uart_2_buffer.tx_buf_head];
    }
    else
    {
        /* No more data for transmission */
...
#endif  /* ((defined UART0) || (defined UART1) || (defined UART2)) */
