 ...
 * @file pal_timer.c
 *
 * @brief Timer related functions for the ATmega256RFR2 Mega RF Single Chip
 *
 * This file implements timer related functions for the ATmega256RFR2.
 *
 * $Id: pal_timer.c 29425 2011-11-18 13:06:06Z sschneid $
 *
...
#ifdef ENABLE_RTB
    /*
     * In case the RTB is enabled for single chips supporting this
     * functionality, futher timer initialization is required.
     */
 ...
 *
 * Unused for the ATmega256RFR2 because the MAC symbol counter
...
void  pal_timer_delay(uint16_t delay)
{
     ...
     * different CPU clock frequency, as the ATmega256RFR2 can operate
     ...
     */
#if (F_CPU != 16000000UL) && (F_CPU != 8000000UL) && (F_CPU != 4000000UL) && (F_CPU != 15384600UL)
#  error "pal_timer_delay() is only implemented for F_CPU = 4, 8 and 16 MHz"
#endif
    /*
    ...
    while (delay != 0)
    {
         ...
         */
#if (F_CPU == 16000000)
        /* If the CPU runs at 8MHz, no adjustment is required. */
        actval *= 2 /* microseconds to timer 5 clock cycles @ 2 MHz */;
#endif
#if (F_CPU == 4000000)
        /* If the CPU runs at 4MHz, adjustment is required. */
        actval /= 2 /* microseconds to timer 5 clock cycles @ 500 kHz */;
#endif
        TCNT5 = (uint16_t)(-((int16_t)actval));
        TIFR5 = _BV(TOV5);
        TCCR5B = _BV(CS51);
