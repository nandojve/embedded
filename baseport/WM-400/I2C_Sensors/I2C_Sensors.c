...
#define DEVICE_EEPROM		AT30TSE752_MEM_DEVICE
...
int main(void)
{
	...
	ioport_set_pin_dir(PIN_PA06, IOPORT_DIR_OUTPUT);		// Enable WM-400L PIN as output
	...
	switch(*state)
	{
		case TS_TSE_LOOP:
		{
			...
			//{
				LED_On(LED1);
				int16_t				itemp		= 0;
				status_code_t		ret			= at30tse752_read_register(AT30TSE52X_TEMP_REG, (uint16_t *) &itemp);

				if(ret == STATUS_OK)
				{
					printf("TSE752 Temp: %f °C\n", itemp, at30tse752_itemp_to_ftemp(itemp));
				}
				else
					printf("TSE752 fail!\n");
				
				ret								= cc2d33s_start_convertion();
